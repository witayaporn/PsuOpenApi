...
async def find_comment(subjectId: str, request: Request):
    try:
        ...
        async def nested_comment(comments):
        ...
        nested_comments = await nested_comment(comments)
        return nested_comments
    except :
        pass
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Comment with ID {subjectId} not found")
...
async def create_comment(request: Request):
    ...
    comment['parentId'] = ObjectId(comment['parentId']) if len(comment['parentId']) else None
...
@router_comment.put("/{subjectId}", response_description="Update a Comment", response_model=Comment)
async def update_comment(subjectId: str, request: Request, comment: CommentUpdate = Body(...)):
    ...
    if (
        existing_comment := request.app.database["Comment"].find_one({"subjectId": subjectId})
    ) is not None:
