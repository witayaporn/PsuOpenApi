...
@router_student.get(
    "/", response_description="List all Student", response_model=List[Student]
)
...
@router_student.get(
    "/{studentId}",
    response_description="Get a single student by id",
...
@router_student.post(
    "/",
    response_description="Create a new Student Interest",
...
@router_student.post(
    "/deleteSubjectInterest/{dataId}", response_description="Delete a student interest"
)
async def delete_student(dataId: str, request: Request, response: Response):
    ...
    raise HTTPException(
        status_code=status.HTTP_404_NOT_FOUND,
        detail=f"Book with ID {studentId} not found",
    )


@router_student.get(
    "/getInterestSubject/{subjectId}",
    response_description="Get a student interest by subjectId, year and term",
    response_model=List[Student],
)
async def find_student(
    ...
    raise HTTPException(
        status_code=status.HTTP_404_NOT_FOUND,
        detail=f"Student with ID {subjectId} not found",
    )
